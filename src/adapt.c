...

{
  ...
  for ( i = 0 ; i < ns ; i += 4 ) {
    ...
    /*coordinates on physical triangle*/
    sqt_element_interp_vector(ce, ne, Nk, s, t, 4, y, 3, n, 3, J, work) ;

    func(s[0], t[0], w[0]*J[0]*wt, &(y[3*0]), &(n[3*0]), quad, nc, 0, data) ;
    func(s[1], t[1], w[1]*J[1]*wt, &(y[3*1]), &(n[3*1]), quad, nc, 0, data) ;
    func(s[2], t[2], w[2]*J[2]*wt, &(y[3*2]), &(n[3*2]), quad, nc, 0, data) ;
    func(s[3], t[3], w[3]*J[3]*wt, &(y[3*3]), &(n[3*3]), quad, nc, 0, data) ;
  }
  ...
  /*coordinates on physical triangle*/
  sqt_element_interp_vector(ce, ne, Nk, s, t, ns, y, 3, n, 3, J, work) ;
  
  for ( j = 0 ; j < ns ; j ++ ) {
    func(s[j], t[j], w[j]*J[j]*wt, &(y[3*j]), &(n[3*j]), quad, nc, 0, data) ;
  }
...

{
  ...
  sqt_triangle_divide_loop30(st, st0) ;
  adaptive_quad_kw(ce, ne, Nk, st0, wt, q, nq, func, q0, nc, kwork, data) ;
  sqt_triangle_divide_loop31(st, st1) ;
  adaptive_quad_kw(ce, ne, Nk, st1, wt, q, nq, func, q1, nc, kwork, data) ;
  sqt_triangle_divide_loop32(st, st2) ;
  adaptive_quad_kw(ce, ne, Nk, st2, wt, q, nq, func, q2, nc, kwork, data) ;
  sqt_triangle_divide_loop33(st, st3) ;
  adaptive_quad_kw(ce, ne, Nk, st3, wt, q, nq, func, q3, nc, kwork, data) ;
